...
def validate_title(value):
    ...
    if not re.match(r"^[A-Za-z0-9_ ]+$", value):
        raise serializers.ValidationError(
            "Title can only contain letters, numbers, spaces, and underscore (_)."
        )
    return value
...
def validate_description(value):
    ...
    if not re.match(r"^[A-Za-z0-9_ ]+$", value):
        raise serializers.ValidationError(
            "Description can only contain letters, numbers, spaces, and underscore (_)."
        )
    return value
...
def validate_content(value):
    ...
    if len(text_only.strip()) < 20:
        raise serializers.ValidationError(
            "Content must be meaningful (min 20 characters)."
        )
